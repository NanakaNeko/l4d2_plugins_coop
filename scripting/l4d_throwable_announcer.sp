...
public void OnPluginStart()
{
    ...
    g_hCvar_Team            = CreateConVar("l4d_throwable_announcer_team", "7", "抛出投掷物的提示消息发送给那些团队. 0=没有, 1=幸存者, 2=感染者, 4=旁观者, 8=幸存者NPC. 如需启用多个把数字加起来. 例如: 3=幸存者+感染者.", CVAR_FLAGS, true, 0.0, true, 15.0);
    ...
    switch (type)
    {
        case TYPE_MOLOTOV:
        {
            ...
            for (int client = 1; client <= MaxClients; client++)
            {
                ...
                CPrintToChat(client, "{default}[{green}!{default}] {blue}%N {olive}投掷了燃烧瓶", attacker);
            }
        }
        ...
        case TYPE_PIPEBOMB:
        {
            ...
            for (int client = 1; client <= MaxClients; client++)
            {
                ...
                CPrintToChat(client, "{default}[{green}!{default}] {blue}%N {olive}投掷了土制炸弹", attacker);
            }
        }
