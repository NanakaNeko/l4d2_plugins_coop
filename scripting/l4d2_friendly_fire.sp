...
{
	if (args == 0)
		PrintToChat(client, "\x01[\x04!\x01] \x05请输入 \x04!setff off \x05关闭友伤，\x04!setff on \x05打开友伤，\x04!setff normal \x05设置普通友伤，\x04!setff top \x05设置最高友伤");
	else if (args == 1) {
		char tmp[8];
		...
		if (strcmp(tmp, "on", false) == 0) {
			...
			PrintToChatAll("\x01[\x04!\x01] \x05队友伤害恢复\x03正常\x05伤害.");
		}
			...
			PrintToChatAll("\x01[\x04!\x01] \x05队友伤害设置为\x03关闭.");
		}
			...
			PrintToChatAll("\x01[\x04!\x01] \x05队友伤害设置为\x03普通\x05伤害.");
		}
			...
			PrintToChatAll("\x01[\x04!\x01] \x05队友伤害设置为\x03专家2倍\x05伤害.");
		}
		else
