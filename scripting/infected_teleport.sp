...
public void OnPluginStart()
{
	...
	g_hMaxDistance = CreateConVar("teleport_max_distance", "750.0", "特感传送的位置距离目标生还者的最大距离", CVAR_FLAG, true, 0.0);
	...
	// 日志记录
	//g_hEnableLog = CreateConVar("teleport_log_level", "1", "插件日志记录级别 (1: 禁用, 2: DEBUG, 4: INFO, 8: MESSAGE, 16: SERVER, 32: ERROR) 数字相加", CVAR_FLAG, true, 0.0);
	...
	for(i = 0; i < GetServerTickRate(); i++) {
		...
		if (rayEndNav != Address_Null && 
			...
			if (!IsFakeClient(teleportInfected))
				CPrintToChat(teleportInfected, "{blue}[{default}提示{blue}]: {default}由于您距离生还者过远, 即将将您传送到目标 {green}%N {default}处...", client);
